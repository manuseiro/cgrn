...
import { conformidadeParaTexto } from '../services/conformidade.js';
import { getCoordText } from '../components/ui.js';
...
// ─── Projeto Completo (.cgrn) ─────────────────────────────────────────────
export function exportProject(glebas) {
  ...
  const project = {
    name: document.getElementById('projectName')?.value || 'Projeto CGRN',
    coords,
    glebaCount: glebas.length,
    savedAt: new Date().toISOString(),
    version: '3.5',
    metadata: {
      totalArea: glebas.reduce((s, g) => s + g.area, 0),
    }
  };

  const blob = new Blob([JSON.stringify(project, null, 2)], { type: 'application/json;charset=utf-8;' });
  const name = project.name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  triggerDownload(URL.createObjectURL(blob), `${name}_${ts()}.cgrn`);
  showToast(`<i class="bi bi-file-earmark-code"></i> Projeto exportado com sucesso.`, 'success');
}
